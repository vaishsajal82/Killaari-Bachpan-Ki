...
class GalleryItem(Base):
    ...
    display_order = Column(Integer, default=0)
    # Cloudinary's asset identifier (e.g. "kilkaari/uploads/abc123"), not
    # shown anywhere in the UI. Only present for images uploaded through
    # the Cloudinary flow — nullable so existing rows with an old
    # locally-hosted image_url (see main.py's backward-compat static
    # mount) or a manually-pasted external URL are unaffected. Used
    # exclusively so the old Cloudinary asset can actually be deleted when
    # an item's image is replaced or the item itself is deleted, instead
    # of silently orphaning it — see app/routers/gallery.py.
