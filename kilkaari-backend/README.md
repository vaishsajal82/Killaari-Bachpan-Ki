...
- **Image storage:** Cloudinary (see "Image uploads" below)
- **Rate limiting:** slowapi, per-endpoint (login, contact form, donations, etc.)
...
`POST /api/uploads` (admin-only, multipart file upload) validates the file
is a real JPEG/PNG/GIF/WEBP by its actual bytes (not the filename or
Content-Type header — see `app/image_validation.py`), then uploads it to
Cloudinary and returns `{"url", "public_id", "width", "height", "format"}`.
Used by every content type's image field in the admin portal.

Requires `CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, `CLOUDINARY_API_SECRET`
in `.env` — without them, the endpoint returns a clean `503` rather than
crashing. Gallery items additionally store `cloudinary_public_id` so their
old image is properly deleted from Cloudinary when replaced or the item is
deleted (see `app/routers/gallery.py`); other content types don't track
this yet, so their old Cloudinary assets aren't automatically cleaned up.

Images uploaded before this migration (served from `/uploads/<file>` on
this server) keep working — that static route is kept for backward
compatibility even though nothing writes to it anymore.

## Security
- **Rate limiting:** login (10/min), register/contact/volunteer (5/min),
  newsletter/donations (10/min) — per client IP, in-memory (see
  `app/rate_limit.py`)
- **Security headers:** CSP, HSTS, X-Frame-Options, X-Content-Type-Options,
  Referrer-Policy, Permissions-Policy on every response except `/docs`/`/redoc`
  (which need a looser policy for Swagger UI's CDN assets to load)
- **CORS:** defaults to `*` if `CORS_ORIGINS` is unset (safe here since auth
  is Bearer-token-only, never cookies — see the comment in `app/main.py`);
  set it to your real frontend domain(s) for tighter production config
- **Errors:** unhandled exceptions never leak a stack trace, DB error, or
  file path to the client — see the global exception handlers in `app/main.py`
- **Dependency scanning:** `../scripts/security-audit.sh` runs `pip-audit`
  against `requirements.txt`; also runs automatically in CI on every push
  (`.github/workflows/security-audit.yml`)

## Connecting the frontend
The public site (`Kilkaari/`) and admin portal (`kilkaari-admin/`) already
make real `fetch()` calls to this API — see `Kilkaari/assets/js/api.js` and
`kilkaari-admin/assets/js/admin.js`. Both currently point at the deployed
Render URL by default; override it locally by setting
`window.KILKAARI_API_BASE_URL` before those scripts load, or point
`CORS_ORIGINS` in this backend's `.env` at wherever you're serving the
frontend from if you're running everything locally together.
