  ...
  async function syncStars() {
    ...
    grid.innerHTML = published
      .map((s) => {
        ...
        const frameStyle = hasPhoto ? ` style="background-image:url('${s.image_url}');"` : '';
        const frameClass = hasPhoto ? 'photo-frame has-photo' : 'photo-frame tone-blue';
        const link = s.story_url
          ? `<a href="${s.story_url}" target="_blank" rel="noopener" class="program-card__link">See My Story <i class="fa-solid fa-arrow-up-right-from-square"></i></a>`
          : '';
        return `
    ...
    track.innerHTML = published
      .map((t) => {
        const avatarInner = t.avatar_url
          ? `<img src="${t.avatar_url}" alt="${escapeHtml(t.name)}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`
